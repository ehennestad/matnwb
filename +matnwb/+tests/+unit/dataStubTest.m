...
timeseries = matnwb.types.core.TimeSeries(...
...
rc = matnwb.types.rrs.RefContainer('data', matnwb.types.rrs.RefData('data', rand(100, 100)));
rcPath = '/acquisition/rc';
rcDataPath = [rcPath '/data'];
rcRef = matnwb.types.cs.CompoundRefData('data', table(...
    rand(2, 1),...
    rand(2, 1),...
    [matnwb.types.untyped.ObjectView(rcPath); matnwb.types.untyped.ObjectView(rcPath)],...
    [matnwb.types.untyped.RegionView(rcDataPath, 1:2, 99:100); matnwb.types.untyped.RegionView(rcDataPath, 5:6, 88:89)],...
...
matnwb.tests.util.verifyContainerEqual(testCase, nwbNew, nwb);
