classdef PatchClampSeries < matnwb.types.core.TimeSeries & matnwb.types.untyped.GroupClass
...
methods
    function obj = PatchClampSeries(varargin)
        ...
        if strcmp(class(obj), 'matnwb.types.core.PatchClampSeries')
    ...
    function val = validate_electrode(obj, val)
        val = matnwb.types.util.checkDtype('electrode', 'matnwb.types.core.IntracellularElectrode', val);
    end
    function val = validate_gain(obj, val)
        val = matnwb.types.util.checkDtype('gain', 'single', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
    ...
    function val = validate_stimulus_description(obj, val)
        val = matnwb.types.util.checkDtype('stimulus_description', 'char', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
    ...
    function val = validate_sweep_number(obj, val)
        val = matnwb.types.util.checkDtype('sweep_number', 'uint32', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
    ...
    function refs = export(obj, fid, fullpath, refs)
        ...
        matnwb.io.writeAttribute(fid, [fullpath '/stimulus_description'], obj.stimulus_description);
        if ~isempty(obj.sweep_number)
            matnwb.io.writeAttribute(fid, [fullpath '/sweep_number'], obj.sweep_number);
        end
    end
end
