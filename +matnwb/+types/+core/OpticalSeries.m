...
methods
    function obj = OpticalSeries(varargin)
        ...
        if strcmp(class(obj), 'matnwb.types.core.OpticalSeries')
    ...
    function val = validate_distance(obj, val)
        val = matnwb.types.util.checkDtype('distance', 'single', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
    ...
    function val = validate_orientation(obj, val)
        val = matnwb.types.util.checkDtype('orientation', 'char', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
