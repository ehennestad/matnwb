classdef Image < matnwb.types.core.NWBData & matnwb.types.untyped.DatasetClass
...
methods
    function obj = Image(varargin)
        ...
        if strcmp(class(obj), 'matnwb.types.core.Image')
    ...
    function val = validate_resolution(obj, val)
        val = matnwb.types.util.checkDtype('resolution', 'single', val);
        if isa(val, 'matnwb.types.untyped.DataStub')
            if 1 == val.ndims
                valsz = [val.dims 1];
            else
