function checkConstraint(pname, name, namedprops, constrained, val)
    ...
    if any(strcmp(name, names))
        matnwb.types.util.checkDtype([pname '.' name], namedprops.(name), val);
        return;
    end
    ...
    for i=1:length(constrained)
        ...
        try
            matnwb.types.util.checkDtype([pname '.' name], allowedType, val);
            return;
        catch ME
            expectedErrorTypes = {...
    ...
    error('NWB:CheckConstraint:InvalidType',...
        ...
        pname, name, matnwb.misc.cellPrettyPrint(constrained), class(val));
end