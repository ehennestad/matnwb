...
for i = 1:length(newColNames)
    ...
    if 8 == exist('matnwb.types.hdmf_common.VectorIndex', 'class')
        if ~isa(new_cv, 'matnwb.types.hdmf_common.VectorIndex')
...
function indexName = getIndexInSet(inputStruct, inputName)
    ...
    T = matnwb.types.hdmf_common.DynamicTable();
    ...
    indexName = matnwb.types.util.dynamictable.getIndex(T, inputName);
end
