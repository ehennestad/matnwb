    ...
    for i=1:length(propnames)
        ...
        if isempty(validationBody)
            ...
            funcstionStr = strjoin({headerStr ...
                matnwb.file.addSpaces(strtrim(validationBody), 4) 'end'}, newline);
        end
...
function unitValidationStr = fillUnitValidation(name, prop, namespaceReg)
    ...
    if ~isscalar(prop)
        ...
        unitValidationStr = strjoin({...
            ...
            ['matnwb.types.util.checkSet(''', name, ''', struct(), constrained, val);'], ...
            }, newline);
    elseif isa(prop, 'matnwb.file.Dataset')
        unitValidationStr = fillDatasetValidation(name, prop, namespaceReg);
    elseif isa(prop, 'matnwb.file.Group')
...
function unitValidationStr = fillGroupValidation(name, prop, namespaceReg)
    ...
    unitValidationStr = strjoin({...
        ...
        ['matnwb.types.util.checkSet(''', name, ''', namedprops, constrained, val);'], ...
        }, newline);
end
...
function fdvstr = fillDimensionValidation(type, shape)
    ...
    fdvstr = strjoin({...
        ...
        'matnwb.types.util.checkDims(valsz, validshapes);'}, newline);
end
...
function fdvstr = fillDtypeValidation(name, type)
    if isstruct(type)
        ...
        fdvstr = [fdvstr, newline, strjoin(vprops, newline), newline, ...
            'val = matnwb.types.util.checkDtype(''' name ''', vprops, val);'];
    else
        fdvstr = '';
        ...
        fdvstr = [fdvstr ...
            'val = matnwb.types.util.checkDtype(''' name ''', ''' ts ''', val);'];
    end
end
